...
Handle Forward_BuildBuyMenu = null;
Handle Forward_BuyMenuSelect = null;
...
void createNatives(){
	...
	Forward_BuildBuyMenu = CreateGlobalForward("OnBuyMenuBuilding", ET_Ignore, Param_Cell, Param_Cell);
	Forward_BuyMenuSelect = CreateGlobalForward("OnBuyMenuSelect", ET_Ignore, Param_Cell, Param_Cell, Param_Cell);
...
BuildBuyMenu(client)
{
	...
	{
		char localizedText[40];
		new Handle:menu = CreateMenu(TopMenu);
		if(GetConVarInt(CategoriesEnabled[CategoryWeapons]) == 1)
		{
			Format(localizedText, sizeof(localizedText), "%T", "Weapons", client);
			AddMenuItem(menu, "g_WeaponsMenu", localizedText);
		}
		if(GetConVarInt(CategoriesEnabled[CategoryUpgrades]) == 1)
		{
			Format(localizedText, sizeof(localizedText), "%T", "Upgrades", client);
			AddMenuItem(menu, "g_UpgradesMenu", localizedText);
		}
		if(GetConVarInt(CategoriesEnabled[CategoryHealth]) == 1)
		{
			Format(localizedText, sizeof(localizedText), "%T", "Health", client);
			AddMenuItem(menu, "g_HealthMenu", localizedText);
		}
		
		// Menu extension
		Action result;	// We dont care about the result
		Call_StartForward(Forward_BuildBuyMenu);
		Call_PushCell(client);
		Call_PushCell(menu);
		Call_Finish(result);
		
		Format(localizedText, sizeof(localizedText), "%T", "Points Left", client, PlayerData[client][iPlayerPoints]);
		SetMenuTitle(menu, localizedText);
	...
	switch(action)
	{
		...
		case MenuAction_Select:
		{
			...
			else if(StrEqual(menu1, "g_UpgradesMenu"))
			{
				...
				// Menu extension
				Action result;	// We dont care about the result
				Call_StartForward(Forward_BuyMenuSelect);
				Call_PushCell(param1);	// Client ID
				Call_PushCell(menu);	// The menu handle
				Call_PushCell(param2);	// Item Index
				Call_Finish(result);
			}
		}
	}
